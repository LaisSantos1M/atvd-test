...
import { validateEmail, validateId } from "../src/helpers/validateData";
import {faker} from "@faker-js/faker"
import assert from "node:assert";

describe("Testes do validateEmail:", () => {
    test("Deve retornar true para emails validos", () =>{
        const response = validateEmail(faker.internet.email());

        assert.deepStrictEqual(response, true);
    });

    test("Dev retornar false para emails sem @", () => {
...
describe("Testes do validadeId:", () =>{
    test("Deve retornar true para id validos", () =>{
        const randomInt = faker.number.int();
    });

    test("Deve retornar false para id que não é numero", () =>{
   ...
   test("Deve retornar false para id 0", () =>{
    ...
    test("Deve retornar false para id negativo", () =>{
       ...
       test("Deve retornar false para decimal", () =>{
